...
class GlobalSearch extends Component
{
    ...
    {
        $this->history = Cache::tags(['search_history'])->get($this->getUserCacheKey(), []);
    }
    ...
    private function performSearch(): void{
        $cacheKey = $this->generateSearchCacheKey();

        $this->results = Cache::tags(
            ['search_results'])->remember(
                $cacheKey,
                self::CACHE_DURATION,
    ...
    private function generateSearchCacheKey(): string{
        return sprintf(
            'search:%s:%s',
            md5($this->query),
    ...
    {
        $this->history = [];
        Cache::tags(['search_history'])->forget($this->getUserCacheKey());
    }
